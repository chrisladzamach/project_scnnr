...
def modificar_comentario(archivo, linea, nuevo_comentario):
  if not solicitar_contraseña():
    error("Contraseña incorrecta. No se realiazron cambios.")
    return
...
def eliminar_comentario(archivo, linea):
  if not solicitar_contraseña():
    error("Contraseña incorrecta. No se realiazdon cambios.")
    return
  ...
  try:
    ...
    if linea < 1 or linea > len(lineas):
      error("Número de líneas fuera de rango.")
      return
    ...
    succes(f"Comentario en la línea {linea} eliminado.")
